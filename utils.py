...
class EmbeddingLoader(object):
    ...
    def __build_vocab(self):
        ...
        for file in file_type:
            label_file = '{}.json'.format(file)
            print('building vocabulary from %s' % label_file)
...
class RelationLoader(object):
    ...
    def __load_relation(self):
        ...
        with open(relation_file, 'r', encoding='utf-8') as fr:
            rel2id = json.load(fr)
        id2rel = {num: rel for num, rel in enumerate(rel2id)}
...
if __name__ == '__main__':
    ...
    config = Config()
    rl = RelationLoader(config)
    rel2id, id2rel, class_num = rl.get_relation()

    el = EmbeddingLoader(config)
    token2id, emb = el.load_embedding()

    corpus = NYTCorpus(rel2id, token2id, config)
